...
function App() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Error en el servidor. Intenta nuevamente.');
      }
  ...
  const loadDemoAccount = (account) => {
    ...
    setMessage(`✅ Cuenta ${account.role} cargada. Haz clic en "Iniciar Sesión"`);
  };
  ...
  const trackPackage = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:3001/api/packages/tracking/${trackingCode}`, {
  ...
  const getMyDeliveries = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:3001/api/packages/messenger/my-deliveries', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const getMyPackages = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:3001/api/packages/client/my-packages', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const assignMessenger = async (packageId, messengerId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:3001/api/packages/${packageId}/assign-messenger`, {
        method: 'PUT',
        headers: {
  ...
  const createPackage = async () => {
    try {
      if (!newPackage.sender_name || !newPackage.recipient_name || !newPackage.delivery_address) {
        setMessage('❌ Faltan campos obligatorios');
        return;
      }

      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:3001/api/packages/register', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const updatePackageStatus = async (packageId, newStatus) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:3001/api/packages/${packageId}/status`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
            ...
            <div className="login-card">
              ...
              {loginError && loginError.includes('servidor') && (
                <div className="error-message server-error">
                  ...
                  <p>Estamos experimentando problemas técnicos. Por favor, intente nuevamente.</p>
                </div>
              )}
              ...
              <div className="test-accounts">
                ...
                <div className="universal-password">
                  <p><strong>Contraseña universal para testing:</strong> password</p>
                </div>
              </div>
  ...
  return (
    <div className="App">
      <header className="App-header">
            ...
            {trackingData && (
              <div className="tracking-result">
                ...
                <div className="tracking-details">
                  <p><strong>Código:</strong> {trackingData.tracking_code}</p>
                  <p><strong>Remitente:</strong> {trackingData.sender_name}</p>
                  <p><strong>Destinatario:</strong> {trackingData.recipient_name}</p>
                  <p><strong>Dirección:</strong> {trackingData.delivery_address}</p>
                  <p><strong>Estado:</strong> 
                    <span className={`status ${trackingData.status.toLowerCase().replace(' ', '-')}`}>
                      {trackingData.status}
                    </span>
                  </p>
                  <p><strong>Ubicación:</strong> {trackingData.current_location}</p>
                  {trackingData.messenger_name && (
                  <p><strong>Mensajero:</strong> {trackingData.messenger_name}</p>
                  )}
                  <p><strong>Costo:</strong> ${parseFloat(trackingData.cost).toLocaleString()}</p>
                </div>
              </div>
            )}
          </div>
        ...
        {activeTab === 'management' && (currentUser.role === 'operator' || currentUser.role === 'admin') && (
          <div className="tab-content">
            <h2>⚙️ Gestión Completa de Envíos</h2>
            <p><em>Vista de {currentUser.role} - Gestión de todos los envíos del sistema</em></p>
                ...
                <div className="modal-content">
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) => setNewPackage({...newPackage, delivery_address: e.target.value})}
                  ...
                  <div className="form-actions">
                    ...
                    <button onClick={() => setShowCreatePackage(false)}>❌ Cancelar</button>
                  </div>
                </div>
              </div>
            )}
                  ...
                  {allPackages.map(pkg => (
                    <div key={pkg.id} className="package-management-card card">
                      <div className="package-header">
                        ...
                        <span className={`status ${pkg.status.toLowerCase().replace(' ', '-')}`}>
                          {pkg.status}
                        </span>
                      </div>
                      ...
                      <div className="package-actions">
                        ...
                        <button onClick={() => updatePackageStatus(pkg.id, 'in_transit')}>
                          🚚 En Tránsito
                        </button>
                        <button onClick={() => updatePackageStatus(pkg.id, 'out_for_delivery')}>
                          📦 En Entrega
                        </button>
